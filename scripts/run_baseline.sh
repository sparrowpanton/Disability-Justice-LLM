#!/bin/bash
# Run baseline testing — execute from MadTheologyLLM directory
...
OUTFILE="data/baseline/baseline_$(date +%Y%m%d_%H%M%S).jsonl"
LOGFILE="data/baseline/run_log.txt"
...
# Prompt format: ID|category|case|prompt_text
PROMPTS=(
...
for MODEL_STR in "${MODELS[@]}"; do
    ...
    for PROMPT_STR in "${PROMPTS[@]}"; do
        ...
        # Clean response for JSON (escape quotes and newlines)
